    ...
    while let Some(c) = w.next_opt() {
        match c {
            ...
            'v' => {
                printf!(
                    "%s",
                    wgettext_fmt!("%s, version %s\n", PACKAGE_NAME, fufufafa::BUILD_VERSION)
                );
                return ControlFlow::Break(0);
            }
            'D' => {
...
fn throwing_main() -> i32 {
    ...
    fufufafa::localization::initialize_localization();
    ...
    fufufafa::env_dispatch::read_terminfo_database(EnvStack::globals());
    proc_init();
    reader_init(true);
...
// https://github.com/levouinse/fufufafa-shell/issues/367
