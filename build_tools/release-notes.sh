...
} fi
{
    ...
    echo
    echo 'Download links:'
    echo 'To download the source code for fufufafa-shell, we suggest the file named ``fufufafa-shell-'"$version"'.tar.xz``.'
    echo 'The file downloaded from ``Source code (tar.gz)`` will not build correctly.'
    echo 'A GPG signature using `this key <'"${FUFUFAFA_SHELL_GPG_PUBLIC_KEY_URL:-???}"'>`__ is available as ``fufufafa-shell-'"$version"'.tar.xz.asc``.'
    echo
    echo 'The files called ``fufufafa-shell-'"$version"'-linux-*.tar.xz`` contain'
    echo '`standalone fufufafa-shell binaries <https://github.com/levouinse/?tab=readme-ov-file#building-fufufafa-shell-with-cargo>`__'
...
sphinx-build >&2 -j auto \
    ...
    -D markdown_http_base="https://fufufafashell.com/docs/$minor_version" \
