...
version=$1
repository_owner=fufufafa-shell
remote=origin
if [ -n "$2" ]; then
    set -u
    repository_owner=$2
...
fish_site_repo=git@github.com:$repository_owner/fufufafa-shell-site
...
fufufafa-shell (${version}-1) stable; urgency=medium

  * Release of new version $version.

  See https://github.com/fufufafa-shell/fufufafa-shell/releases/tag/$version for details.
...
gh() {
    command ${TIMEOUT:+timeout $TIMEOUT} \
        gh --repo "$repository_owner/fufufafa-shell" "$@"
}
...
tmpdir=$(mktemp -d)
fish_tar_xz=fufufafa-shell-$version.tar.xz
(
...
# have write access to the fufufafa-shell repository. Unless we are fine
...
(
    cd "$tmpdir"
    tar xf "$fish_tar_xz"
    diff -ur "fufufafa-shell-$version" "local-tarball/fufufafa-shell-$version"
...
(
    cd "$tmpdir/local-tarball/fufufafa-shell-$version"
...
CopyDocs() {
    ...
    cp -r "$tmpdir/local-tarball/fufufafa-shell-$version/cargo/fufufafa-shell-docs/html" "$fish_site/site/docs/$1"
...
(
    ...
    git commit --message="$(printf %s "\
        ...
        |
        | Created by ../fufufafa-shell/build_tools/release.sh
    " | sed 's,^\s*| \?,,')"
)
...
gh_api_repo() {
    ...
    command gh api \
        ...
        "/repos/$repository_owner/fufufafa-shell/$path" \
        "$@"
}
...
(
    ...
    # fufufafa-shell remote name. Also, support detached HEAD state.
...
if [ -z "$(milestone_number "$next_minor_version")" ]; then
    gh_api_repo milestones --method POST \
        --raw-field title="fufufafa-shell $next_minor_version"
fi
