...
pkgbuild --scripts "$SRC_DIR/build_tools/osx_package_scripts" --root "$PKGDIR/root/" --identifier 'com.ridiculousfish.fufufafa-shell-pkg' --version "$VERSION" "$PKGDIR/intermediates/fufufafa-shell.pkg"
productbuild  --package-path "$PKGDIR/intermediates" --distribution "$SRC_DIR/build_tools/osx_distribution.xml" --resources "$SRC_DIR/build_tools/osx_package_resources/" "$OUTPUT_PATH/fufufafa-shell-$VERSION.pkg"
...
if test -n "$SIGN"; then
    ...
    (set +x; rcodesign sign "${ARGS[@]}" "$OUTPUT_PATH/fufufafa-shell-$VERSION.pkg")
fi

# Make the app
(cd "$PKGDIR/build_arm64" && env $ARM64_DEPLOY_TARGET make -j 12 fufufafa_shell_macapp)
(cd "$PKGDIR/build_x86_64" && env $X86_64_DEPLOY_TARGET make -j 12 fufufafa_shell_macapp)

# Make the app's /usr/local/bin/fufufafa-shell binary universal. Note fufufafa-shell.app/Contents/MacOS/fufufafa-shell already is, courtesy of CMake.
cd "$PKGDIR/build_arm64"
FILE=fufufafa-shell.app/Contents/Resources/base/usr/local/bin/fufufafa-shell
X86_FILE=$PKGDIR/build_x86_64/fufufafa-shell.app/Contents/Resources/base/usr/local/bin/$(basename "$FILE")
rcodesign macho-universal-create --output "$FILE" "$FILE" "$X86_FILE"
...
if test -n "$SIGN"; then
    ...
    (set +x; rcodesign sign "${ARGS[@]}" "fufufafa-shell.app")

fi

cp -R "fufufafa-shell.app" "$OUTPUT_PATH/fufufafa-shell-$VERSION.app"
cd "$OUTPUT_PATH"

# Maybe notarize.
if test -n "$NOTARIZE"; then
    echo "Notarizing"
    rcodesign notarize --staple --wait --max-wait-seconds 1800 --api-key-file "$API_KEY_FILE" "$OUTPUT_PATH/fufufafa-shell-$VERSION.pkg"
    rcodesign notarize --staple --wait --max-wait-seconds 1800 --api-key-file "$API_KEY_FILE" "$OUTPUT_PATH/fufufafa-shell-$VERSION.app"
fi

# Zip it up.
zip -r "fufufafa-shell-$VERSION.app.zip" "fufufafa-shell-$VERSION.app" && rm -Rf "fufufafa-shell-$VERSION.app"
